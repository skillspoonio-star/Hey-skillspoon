...
export default function TakeawayConfirmationPage() {
    ...
    let mounted = true
      ; (async () => {
        try {
          ...
          if (!res.ok) {
            console.error('Failed to fetch takeaway order', res.status)
            return
          }
  ...
  return (
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      ...
      <Card className="w-full max-w-lg border-2 border-green-500 shadow-xl">
        <CardContent className="p-6">
          <div className="flex flex-col items-center text-center space-y-3">
            ...
            <div className="space-y-2">
              ...
              <p className="text-green-700">Your takeaway order has been placed successfully.</p>
            </div>

            <div className="w-full bg-green-50 rounded-lg p-3 space-y-1">
              ...
              <div className="text-sm text-green-700">
                Payment: {orderDetails?.paymentStatus === 'paid' ? '✅ Paid' : '💵 Cash on Pickup'}
              </div>
            </div>
          </div>
          ...
          <div className="mt-4 p-3 bg-orange-50 rounded-lg border border-orange-200">
            <div className="flex items-center gap-2 mb-2">
              ...
              <p className="text-sm text-orange-900 font-medium">
                🍽️ Ready for pickup in 30 minutes
              </p>
            </div>
          ...
          <div className="mt-4 p-3 bg-amber-50 rounded-lg border border-amber-200">
            <h4 className="font-bold text-amber-900 mb-2">📋 Pickup Instructions:</h4>
            <ul className="text-xs text-amber-800 space-y-1">
              <li>• Arrive within 15 minutes of ready time</li>
              <li>• Call us if you're running late</li>
              <li>• Have your order ID ready</li>
              {orderDetails?.paymentStatus !== 'paid' && (
                <li>• Bring exact change for cash payment</li>
              )}
            </ul>
          </div>
